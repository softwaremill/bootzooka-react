...
describe('structure', () => {
  it('should contain currentPassword input', () => {
    const wrapper = shallow(<PasswordDetails authService={authService} notifyError={notifyError} notifySuccess={notifySuccess} />);
...
describe('behaviour', () => {
  it('an error should appear under empty currentPassword input on blur', () => {
    ...
    expect(wrapper.contains(<p className="validation-message" key={0}>current password is required!</p>)).toBe(true);
  });
  ...
  it('should call authService.changePassword fn when form is submitted', async () => {
    const wrapper = shallow(<PasswordDetails authService={authService} notifyError={notifyError} notifySuccess={notifySuccess} />);
    const initialState = wrapper.state();
    wrapper.setState({ ...initialState, values: { currentPassword: 'blabla', newPassword: 'P4sSW0Rd#1', repeatedNewPassword: 'P4sSW0Rd#1' } });
